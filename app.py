...
# Optional PyMuPDF (fitz) Import for Catalog & PDF Handling
try:
    import fitz  # PyMuPDF
...
def extract_invoice_data_multiformat(file_bytes, mime_type="image/jpeg"):
    ...
    try:
        if "pdf" in mime_type.lower():
            ...
            buffer.seek(0)
            contents = [Image.open(BytesIO(buffer.getvalue()))]
        ...
        4. "Line Items": Extract every product row accurately across all pages.
...
def parse_single_catalog_file(file_tuple):
    ...
    try:
        if "pdf" in mime_type.lower():
            ...
            buf.seek(0)
            contents = [Image.open(BytesIO(buf.getvalue()))]

        cat_prompt = """
        You are an enterprise catalog OCR system for building materials, laminates, door skins, plywood, and hardware.
        Extract the Supplier/Brand Company Name (e.g. DAARVI, GREENPLY, EBCO) from the page header, and every product row listed across all pages in the price list.

        CRITICAL EXTRACTION RULES:
        1. "Supplier Name": Brand or company name publishing this price list.
        2. "Products": List of all items with:
            - "Item Name": Full product title or description.
            - "Model Code": Model number or series code if present, else "".
            - "Unit": PCS, BOX, SET, LTR, KG, SQFT, MTR. Default "PCS".
            - "GST Rate": GST percentage (0, 5, 12, 18, 28). Default 18.0.
            - "Dealer Rate": Printed rate per piece or MRP as printed under RATE PER PCS / PRICE column.
        ...
        raise Exception(f"Catalog parsing failed: {e}")
