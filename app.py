...
with st.sidebar.expander("✏️ Rename Active Store", expanded=False):
  ...
  renamed_input = st.text_input(
      "New Name:", value=current_display, key="rename_input_field"
  )
...
with st.sidebar.expander("➕ Register New Store", expanded=False):
  new_store_name = st.text_input(
      "Store Title:", placeholder="e.g. Metro Hardware", key="add_input_field"
  )
...
if not api_key:
  st.error(
      "⚠️ Gemini API Key missing. Please configure GEMINI_API_KEY in secrets"
...
with header_left:
  ...
  st.caption(
      "Commercial Multi-Store AI Purchase Intake & Inventory Synchronizer"
  )
...
master_sku_list = (
    master_df["Official_SKU_Name"].tolist() if not master_df.empty else []
)
...
def match_sku(raw_name):
  ...
  if master_sku_list:
    match, score, _ = process.extractOne(
        raw_name, master_sku_list, processor=utils.default_process
    )
    if score > 65:
...
def extract_invoice_data(image):
  ...
  prompt = """
    ...
    1. "Unit Price (Excl. Tax)": Per-item rate before GST if printed.
    2. "Line Total Taxable Amount": The total base amount before GST for this item line (Printed Amount column).
  ...
  # VALID PRODUCTION MODEL CANDIDATES
...
with tab_parser:
  ...
  if uploaded_files:
    st.write("")
    if st.button(
        "🚀 Process Invoices with Fast AI Engine",
        type="primary",
      ...
      with st.status(
          "Parsing purchase bills concurrently with Multimodal AI...",
          expanded=True,
      ) as status_container:
        with ThreadPoolExecutor(
            max_workers=min(len(file_bytes_list), 10)
        ) as executor:
          raw_results = list(
              executor.map(process_single_file_raw, file_bytes_list)
          )
        ...
        for parsed_json in raw_results:
          ...
          supplier = parsed_json.get(
              "Supplier Company Name", "Unknown Supplier"
          )
          ...
          for row in parsed_json.get("Line Items", []):
            ...
            # RECONCILIATION MATH WITH CORRECT PRIORITY
            if unit_price > 0:
            ...
            all_parsed_items.append({
                ...
                "Purchase Price": round(
                    final_base, 2
                ),  # Base Excl. GST Rate for ERP Import
        ...
        status_container.update(
            label="✅ Ingestion & Batch Extraction Complete!",
            state="complete",
            expanded=False,
        )
  ...
  if "parsed_df" in st.session_state:
    ...
    st.caption(
        "Verify AI extraction, mapped SKUs, and rate details before exporting"
        " to accounting software."
    )

    df = st.session_state["parsed_df"]

    df["Line Total (Excl. GST)"] = (
        df["Purchase Price"] * df["Current Quantity"]
    ).round(2)
    df["GST Tax Amount"] = (
        df["Line Total (Excl. GST)"] * (df["GST Rate"] / 100)
    ).round(2)
    df["Line Total (Incl. GST)"] = (
        df["Line Total (Excl. GST)"] + df["GST Tax Amount"]
    ).round(2)

    df["Unit Cost (GST Paid) ₹"] = (
        df["Line Total (Incl. GST)"] / df["Current Quantity"]
    ).round(2)
    ...
    uom_summary_str = " | ".join(
        [f"{val:,.2f} {unit}" for unit, val in uom_groups.items()]
    )
    ...
    m4.metric(
        "Grand Total (GST Paid)",
        f"₹{grand_total_incl_tax:,.2f}",
        delta=f"GST Tax: ₹{total_gst:,.2f}",
    )
        ...
        column_config={
            "Official SKU": (
                st.column_config.SelectboxColumn(
                    "Official SKU Name", options=master_sku_list, required=True
                )
                if master_sku_list
                else "Official SKU"
            ),
            "Unit Cost (GST Paid) ₹": st.column_config.NumberColumn(
                "Unit Cost (GST Paid) ₹", format="₹%.2f", disabled=True
            ),
            "Purchase Price": st.column_config.NumberColumn(
                "Unit Rate (Excl. GST) ₹", format="₹%.2f"
            ),
            "Line Total (Excl. GST)": st.column_config.NumberColumn(
                "Line Total (Excl. GST) ₹", format="₹%.2f", disabled=True
            ),
            "Line Total (Incl. GST)": st.column_config.NumberColumn(
                "Line Total (Incl. GST) ₹", format="₹%.2f", disabled=True
            ),
            "Selling Price": st.column_config.NumberColumn(
                "Selling Price (Optional) ₹", format="₹%.2f"
            ),
            "Current Quantity": st.column_config.NumberColumn(
                "Current Quantity", min_value=0.1
            ),
            "GST Rate": st.column_config.NumberColumn(
                "GST Rate (%)", min_value=0, max_value=28
            ),
    ...
    st.divider()
    if st.button(
        "✅ Confirm Audit & Generate Excel Import File",
        type="primary",
        use_container_width=True,
    ):
      ...
      st.download_button(
          label=(
              "📥 Download Bulk Import Spreadsheet for"
              f" {ACTIVE_STORE_DISPLAY}"
          ),
    ...
    st.write("")
    with st.expander(
        "📄 Generate Customer Quotation (On-the-go)", expanded=False
    ):
      st.caption(
          "Instantly generate a customer quotation by applying a custom markup"
          " % directly to your final GST-Paid cost."
      )

      col_q1, col_q2 = st.columns(2)
      with col_q1:
        customer_name = st.text_input(
            "Customer Name / Reference", value="Walk-in Customer"
        )
      with col_q2:
      ...
      if st.button("Preview & Download Quotation"):
        ...
        st.metric(
            f"Total Quotation Value (Markup: {markup_pct}% on GST-Paid Cost)",
            f"₹{total_quote_value:,.2f}",
        )
  ...
  with col_add:
    with st.container(border=True):
      ...
      add_unit = st.selectbox(
          "Default Unit", options=["PCS", "BOX", "LTR", "KG", "NOS", "SET"]
      )
      add_gst = st.selectbox(
          "GST Rate (%)", options=[0, 5, 12, 18, 28], index=3
      )
      add_price = st.number_input(
          "Selling Price ₹ (Optional)", min_value=0.0, step=10.0
      )

      if st.button(
          "Save SKU to Catalog", use_container_width=True, type="primary"
      ):
        if add_sku.strip():
